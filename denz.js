...
const { addVote, delVote } = fs.existsSync(`./database/${from}.json`)
...
if (isGroup && !isVote) {
      if (budy.toLowerCase() === "vote") {
        ...
        let _votes = JSON.parse(fs.readFileSync(`./database/${from}.json`));
        ...
        const vote = JSON.parse(fs.readFileSync(`./database/v${from}.json`));
        let _votes = JSON.parse(fs.readFileSync(`./database/${from}.json`));
        let fil = vote.map((v) => v.participant);
        let id_vote = sender ? sender : "6283162388082@s.whatsapp.net";
        if (fil.includes(id_vote)) {
...
const fgi = {
	 ...
	 message: { 
			...
			const addAfkId = (userId, alasane, ddi) => { //by vinz
