...
//			if (!tod.key.fromMe) return
        ...
        denz.on('message-update', async (mek) => {
	try {
		const from = mek.key.remoteJid
	...
	const opt4tag = {
				contextInfo: { mentionedJid: [sender] ,  forwardingScore: 2, isForwarded: true  }
			}
			...
			const body = int.type == 'conversation' ? infoMSG[index].message.conversation : int.type == 'extendedTextMessage' ? infoMSG[index].message.extendedTextMessage.text : int.type == 'imageMessage' ? infoMSG[index].message.imageMessage.caption : int.type == 'stickerMessage' ? 'Sticker' : int.type == 'audioMessage' ? 'Audio' : int.type == 'videoMessage' ? infoMSG[index].videoMessage : infoMSG[index]
	
		const mediaData = int.type === 'extendedTextMessage' ? JSON.parse(JSON.stringify(int.data).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : int.data
			var itsme = `0@s.whatsapp.net`
			if (int.type == 'conversation' || int.type == 'extendedTextMessage') {
				const strConversation = `「 *ANTI REVOKE* 」

*Nama :* ${pushname} ( @${sender.split("@s.whatsapp.net")[0]} )
*Tipe :* Text
...
❏ *_SELFBOT VYN_*`
				denz.sendMessage(from, strConversation, MessageType.text, {quoted: int.data, contextInfo:{mentionedJid:[sender],forwardingScore: 508, isForwarded: true }})
			} else if (int.type == 'stickerMessage') {
				var itsme = `0@s.whatsapp.net`
					var split = `「 *ANTI REVOKE* 」`
					const pingbro23 = {
						contextInfo: {
							participant: itsme,
							quotedMessage: {
				...
				const savedFilename = await denz.downloadAndSaveMediaMessage(int.data, `./src/${filename}`);
				const strConversation = `「 *ANTI REVOKE* 」
...
`
				...
				denz.sendMessage(from, strConversation, MessageType.text, opt4tag, { quoted:int.data})
				denz.sendMessage(from, buff, MessageType.sticker, pingbro23)
				// console.log(stdout)
...
❏ *_SELFBOT VYN_*
`
				denz.sendMessage(from, buff, MessageType.image, { quoted: int.data, contextInfo: { mentionedJid: [sender] }, caption: strConversation })
				fs.unlinkSync(savedFilename)
...
❏ *_SELFBOT VYN_*
`
					denz.sendMessage(from, buff, MessageType.video, { quoted: int.data , contextInfo: { mentionedJid: [sender] }, caption: strConversation, forwardingScore: 2, isForwarded: true })
				fs.unlinkSync(savedFilename)
			} 
...
❏ *_SELFBOT VYN_*
`
				denz.sendMessage(from, strConversation, MessageType.text, {quoted: int.data, contextInfo:{mentionedJid:[sender],forwardingScore: 2, isForwarded: true }})
				denz.sendMessage(from, buff, MessageType.audio, pingbro24)
				fs.unlinkSync(savedFilename)
			}
		}
	} catch (e) {
...
async function kartu(img , judul , desk){
                                      let image = await loadImage("./card.jpg");
                                      let tengah = await loadImage(img);
registerFont("./Piazzolla-Regular.ttf", { family: "piaz" } );
registerFont("./CONSOLAB.ttf", { family: "consolab" } );
ctx.drawImage(image, 0, 0, 450, 600);
ctx.font = "30px 'consolab'";
ctx.fillText(judul, 60, 75)
ctx.font = "14px 'piaz'";
ctx.fillText(desk, 60, 461);
...
const fgi = {
	 ...
	 message: { 
                 ...
                 'jpegThumbnail': dfrply
	                  	...
	                  	const _afk = JSON.parse(fs.readFileSync('./afk.json'))
			
			const addAfkId = (userId, alasane, ddi) => { //by denz
        ddi = moment.tz('asia/jakarta').format('DD/MM/YYYY || HH:mm:ss')
            const obj = {jid: userId, alasan: alasane, time: ddi}
            const repkly = (teks) => {
			 denz.sendMessage(from, teks, text, {quoted:mek})
			}
			
            const push = mek.key.fromMe ? denz.user.name : conts.notify || conts.vname || conts.name || '-'
		
            _afk.push(obj)
            fs.writeFileSync('./afk.json', JSON.stringify(_afk))
        }
const getAfkAlasan = (userId) => {
            let position = false
            Object.keys(_afk).forEach((i) => {
                if (_afk[i].jid === userId) {
                    position = i
                }
            })
            if (position !== false) {
                return _afk[position].alasan
            }
        }
// TEXT WITH THUMBNAIL
const ftex = {
...
denz.sendMessage(id, buttonMessages, MessageType.buttonsMessage, options)
 ...
 if (!mek.key.remoteJid.endsWith('@g.us') && offline){
		  ...
		  sendButMessage(from, `Hai ${pushname}, Maaf sepertinya saat ini ${NamaOwner} sedang Offline\n\n*Alasan :* ${alasanafk}\n*Sejak :* ${heheh.hours} Jam, ${heheh.minutes} Menit, ${heheh.seconds} Detik\n\nSilahkan hubungi lagi setelah Online`, "*_© SneazyBOT_*", [{buttonId: 'simi', buttonText: {displayText: 'Oke'}, type: 1}], {quoted:ftrol, contextInfo: { forwardingScore: 508, isForwarded: true}})
			}
		}
		if (mek.key.remoteJid.endsWith('@g.us') && offline) {
			if (!mek.key.fromMe){
...
console.log(e)
  ...
  if (isGroup && !isVote) {
      if (budy.toLowerCase() === "vote") {
        ...
        let _votes = JSON.parse(fs.readFileSync(`./lib/${from}.json`));
...
if (dbAbsen.expired_on != null && Number(dbAbsen.expired_on) <= moment().unix()) {
	console.log("Expired Absensi");
	var db_absen = JSON.parse(fs.readFileSync(`./database/${from}.json`));
	numberAbs = 1
	teks = `Absensi tanggal *${calender}*\nTotal Yang Absen: ${db_absen.pushabsen.length}\n\n`
	for (let i = 0; i < db_absen.pushabsen.length; i++) {
		teks += `*${numberAbs++}.* ${db_absen.pushabsen[i].name} ( *${db_absen.pushabsen[i].kehadiran}* )\n`
	}
	denz.sendMessage(from, `${teks}\n_Absensi telah selesai, terimakasih._`, text)
	fs.unlinkSync(`./database/${from}.json`);
}
if (fs.existsSync(`./database/${from}.json`)) {
	if (budy.toLowerCase() == 'hadir' || budy.toLowerCase() == 'izin' || budy.toLowerCase() == 'sakit') {
		var db_absen = JSON.parse(fs.readFileSync(`./database/${from}.json`));
		for (let numabsen of db_absen.pushabsen) {
			if (numabsen.number == sender)return reply(`Kamu telah absen sebelumnya!`);
		}
		db_absen.pushabsen.push({
			name: pushname,
			number: sender,
			kehadiran: `${capitalLetter(budy)}`
		})
		fs.writeFile(`./database/${from}.json`, JSON.stringify(db_absen, null, 2),
		(e) => {
			...
			denz.sendMessage(from, `${teks}\nSilahkan ketik *hadir/izin/sakit*\nWaktu Absensi Sampai : ${moment(db_absen.expired_on * 1000).format('HH:mm:ss')} WIB`, text, {quoted: mek})
		})
	}
}
			// MUTE
...
│◦➛${prefix}addchangelog [ teks ]
│◦➛${prefix}delchangelog [ teks ]
│
├─❒ WAR MENU
│◦➛${prefix}virtex
│◦➛${prefix}bugkatalog
...
ENG: PRESS THE BUTTON BELOW TO VERIFY

Join group:
https://chat.whatsapp.com/Ko79Xyt4OJzBV1oyTgCaYF`
gbutsann = [
{buttonId:`verifyok`,buttonText:{displayText:'VERIFY!!'},type:1},
]
mhanz = await denz.prepareMessage(from, ofrply, image, {thumbnail: ofrply})
const sendBtnVeryy = {
imageMessage: mhanz.message.imageMessage,
contentText:`${jancokkab}`,
footerText: @ardynvyn__, 
buttons: gbutsann,
headerType: 4
}
...
{buttonId:`script`,buttonText:{displayText:'INSTAGRAM'},type:1}
]
mhan = await denz.prepareMessage(from, ofrply, image, {thumbnail: ofrply})
const sendBtnVeryy = {
imageMessage: mhan.message.imageMessage,
contentText:`${jancok}`,
footerText:'```BOT CREATED BY © VINZX```\n_Engine: NodeJs 14.x.x_', 
buttons: gbutsan,
headerType: 4
}
denz.sendMessage(from, sendBtnVeryy, MessageType.buttonsMessage, {quoted:ftrol, contextInfo: { mentionedJid: [sender]}})
...
_Note: Gunakan huruf kecil untuk menjawab_`})
						//sendButLocation(from, `Selesaikan captcha ini untuk mendaftar ^-^\n\n_Note: Gunakan huruf kecil untuk menjawab_`, `SNEAZYBOT`, {jpegThumbnail:buffer}, [{buttonId:`${optionsText.text.toLowerCase()}`,buttonText:{displayText:`${optionsText.text.toLowerCase()}`},type:1},{buttonId:`tors`,buttonText:{displayText:'hshsjjs'},type:1},{buttonId:`b7ksh`,buttonText:{displayText:'ygdkw'},type:1}])
					})
				...
				if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
					...
					if (fs.existsSync(`./database/${from}.json`)) {
						fs.unlinkSync(`./database/${from}.json`)
						reply('Berhasil menghapus sesi absen')
					} else {
	...
	case 'revoke':
				...
				if (argz.length === 1) return denz.sendMessage(from, `Penggunaan fitur  :\n\n${prefix}Antirevoke [ enable/disable ] (Untuk grup)\n${prefix} [ ctaktif/ctmati ] (untuk semua kontak)\n${prefix} banct 628558xxxxxxx (banlist kontak)`, text)
				if (argz[1] == 'enable') {
					if (isGroup) {
						if (isRevoke) return denz.sendMessage(from, ` telah diaktifkan di grup ini sebelumnya!`, text)
						dataRevoke.push(from)
						fs.writeFileSync('./src/gc-revoked.json', JSON.stringify(dataRevoke, null, 2))
						denz.sendMessage(from, `antirevoke berhasil di aktifkan!!`, text)
					} else if (!isGroup) {
						denz.sendMessage(from, `Untuk kontak penggunaan ${prefix}antirevoke ctaktif`, text)
					}
				} else if (argz[1] == 'ctaktif') {
					if (!isGroup) {
						if (isCtRevoke) return denz.sendMessage(from, `antirevoke telah diaktifkan di semua kontak sebelumnya!`, text)
						dataCtRevoke.data = true
						fs.writeFileSync('./src/ct-revoked.json', JSON.stringify(dataCtRevoke, null, 2))
						denz.sendMessage(from, ` diaktifkan disemua kontak!`, text)
					} else if (isGroup) {
						denz.sendMessage(from, `Untuk grup penggunaan ${prefix} aktif`, text)
					}
				} else if (argz[1] == 'banct') {
					if (isBanCtRevoke) return denz.sendMessage(from, `kontak ini telah ada di database banlist!`, text)
					if (argz.length === 2 || argz[2].startsWith('0')) return denz.sendMessage(from, `Masukan nomer diawali dengan 62! contoh 62859289xxxxx`, text)
					dataBanCtRevoke.push(argz[2] + '@s.whatsapp.net')
					fs.writeFileSync('./src/ct-revoked-banlist.json', JSON.stringify(dataBanCtRevoke, null, 2))
					denz.sendMessage(from, `Kontak ${argz[2]} telah dimasukan ke banlist  secara permanen!`, text)
				} else if (argz[1] == 'disable') {
					if (isGroup) {
						const index = dataRevoke.indexOf(from)
						dataRevoke.splice(index, 1)
						fs.writeFileSync('./src/gc-revoked.json', JSON.stringify(dataRevoke, null, 2))
						denz.sendMessage(from, `antirevoke berhasil di nonaktifkan!!`, text)
					} else if (!isGroup) {
						denz.sendMessage(from, `Untuk kontak penggunaan ${prefix}antirevoke ctmati`, text)
					}
				} else if (argz[1] == 'ctmati') {
					if (!isGroup) {
						dataCtRevoke.data = false
						fs.writeFileSync('./src/ct-revoked.json', JSON.stringify(dataCtRevoke, null, 2))
						denz.sendMessage(from, ` dimatikan disemua kontak!`, text)
					} else if (isGroup) {
						denz.sendMessage(from, `Untuk grup penggunaan *${prefix}antirevoke mati*`, text)
					}
				}
				break
case 'return':
...
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
                                      try {
                                        ...
                                        var imgbb = require('imgbb-uploader')
                                        run = getRandom('.png')
                                        encmedik = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek
                                        mediawei = await denz.downloadAndSaveMediaMessage(encmedik)
                                        ddatae = await imageToBase64(JSON.stringify(mediawei).replace(/\"/gi, ''))
                                        fs.writeFileSync(`${run}`, ddatae, 'base64')
kartu((fs.readFileSync(`./${run}`)), `${juduls}` , `${desks}`).then((resu) => {
...
case 'whatmusic':
				case 'whatmusik':
				if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
					if ((isMedia && !mek.message.videoMessage || isQuotedAudio || isQuotedVideo) && args.length == 0) {
						try {
							...
							const acr = new acrcloud({
								...
								access_key: "ef00990eadf2cca3dfd678e265e562f4",
								access_secret: "k2r3gKsE7mGbBVU58cYKe5nF6UDzD5LNcb8Rq8Y3"
							});
							sampleq = fs.readFileSync(`./audio.mp3`)
							metadata = await acr.identify(sampleq)
							console.log(metadata)
							denz.sendMessage(from, `*「 Lagu Berhasil Ditemukan 」*\n\n➸ Judul Lagu : ${metadata.metadata.music[0].title}\n➸ Artis : ${metadata.metadata.music[0].artists[0].name}\n➸ Album : ${metadata.metadata.music[0].album.name}\n➸ Rilis : ${metadata.metadata.music[0].release_date}`, text, {quoted: mek})
						} catch (e) {
							console.log(e)
...
/*case 'proxy-checker':
            case 'proxy-check':
                ...
                fs.writeFileSync(`./database/proxy_check_${sender.split('@')[0]}.txt`, args[1])
                proxyChecker.checkProxiesFromFile(
                    `./database/proxy_check_${sender.split('@')[0]}.txt`, {
                        url: 'http://google.com',
                    },
                ...
                setTimeout(function() {
                    Promise.all(live_proxy).then(r => {
                        ...
                        fs.unlinkSync(`./database/proxy_check_${sender.split('@')[0]}.txt`)
                    })
                }, 4000);
                break*/
/*case 'sk':
...
case 'proxy':
                ...
                if (args[1] === 'http') {
                    axios.get('https://api.proxyscrape.com/?request=getproxies&proxytype=http&timeout=5000&country=all&ssl=all&anonymity=all').then(async (r) => {
                        fs.writeFileSync('./database/proxy_http.txt', r.data)
                        var amount_proxy = fs.readFileSync('./database/proxy_http.txt').toString().split('\n').length
                        var last_proxy_update = await axios.get('https://api.proxyscrape.com/?request=lastupdated&proxytype=http')
                        reply(mess.wait)
                        geps.sendMessage(from, fs.readFileSync('./database/proxy_http.txt'), MessageType.document, {
                            quoted: msg,
                            mimetype: 'text/plain',
...
LastUpdate : ${last_proxy_update.data}`)}`, MessageType.text, {
                                ...
                                fs.unlinkSync('./database/proxy_http.txt')
                            })
                        })
                    ...
                    axios.get('https://api.proxyscrape.com/?request=getproxies&proxytype=socks4&timeout=5000&country=all').then(async (r) => {
                        fs.writeFileSync('./database/proxy_socks4.txt', r.data)
                        var amount_proxy = fs.readFileSync('./database/proxy_socks4.txt').toString().split('\n').length
                        var last_proxy_update = await axios.get('https://api.proxyscrape.com?request=lastupdated&proxytype=socks4')
                        reply(mess.wait)
                        geps.sendMessage(from, fs.readFileSync('./database/proxy_socks4.txt'), MessageType.document, {
                            quoted: msg,
                            mimetype: 'text/plain',
...
LastUpdate : ${last_proxy_update.data}`)}`, MessageType.text, {
                                ...
                                fs.unlinkSync('./database/proxy_socks4.txt')
                            })
                        })
                    ...
                    axios.get('https://api.proxyscrape.com/?request=getproxies&proxytype=socks5&timeout=5000&country=all').then(async (r) => {
                        fs.writeFileSync('./database/proxy_socks5.txt', r.data)
                        var amount_proxy = fs.readFileSync('./database/proxy_socks5.txt').toString().split('\n').length
                        var last_proxy_update = await axios.get('https://api.proxyscrape.com?request=lastupdated&proxytype=socks5')
                        reply(mess.wait)
                        geps.sendMessage(from, fs.readFileSync('./database/proxy_socks5.txt'), MessageType.document, {
                            quoted: msg,
                            mimetype: 'text/plain',
...
LastUpdate : ${last_proxy_update.data}`)}`, MessageType.text, {
                                ...
                                fs.unlinkSync('./database/proxy_socks5.txt')
                            })
                        })
...
/*case 'get-bin':
                ...
                break*/
/*case 'shell-checker':
                if (!args[1]) return reply('ngamtuk kah?')
                v
...
var nn = body.slice(9)
		...
		if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
                                          ...
                                          reason = await body.slice(5)
                                          const pushnsame = mek.key.fromMe ? denz.user.name : conts.notify || conts.vname || conts.name || '-'
		
                                          reply(`*「 AFK 」*\nUsername : ${pushnsame}\nAlasan : ${reason}`)
                                          addAfkId(sender, reason)
                                          break
           case 'fitnahpc':
           if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
...
Giliran = @${tty.player1.split('@')[0]}`
 denz.sendMessage(from, ucapan, text, {quoted: mek, contextInfo:{mentionedJid: [tty.player1,tty.player2]}})
	...
	e = String(e)
     ...
     denz.sendMessage(`${settings.NomorOwner}@s.whatsapp.net`, `─────「 *ALERT-ERROR* 」─────\n\n\`\`\`${e}\`\`\`\n\n────────────────────`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "Developer SneazyBOT",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./denz.jpg'),sourceUrl:"https://youtube.com/channel/UCIvY2o-KctmlKWvXzqr"}}})
	}
    }
    }
    }
